...
class Emitter():
    ...
    *    @param typ the type of the constant
    ...
    def emitALOAD(self, in_, frame):
        ...
        elif type(in_) is ArrayPointerType or type(in_) is StringType or type(in_) is cgen.ClassType:
    ...
    def emitASTORE(self, in_, frame):
        ...
        elif type(in_) is ArrayPointerType or type(in_) is StringType or type(in_) is ClassType:
    ...
    *   @param toLabel the ending label  of the scope where the variable is active.
    ...
    def emitREADVAR(self, name, inType, index, frame):
        ...
        elif type(inType) is ArrayPointerType or type(inType) is StringType or type(inType) is cgen.ClassType:
    ...
    *   generate ior
    ...
    def emitREOP(self, op, in_, frame):
        ...
        if op == ">":
            result.append(self.jvm.emitIFLE(labelF)) if type(in_) is FloatType else result.append(self.jvm.emitIFICMPLE(labelF))
        elif op == ">=":
            result.append(self.jvm.emitIFLT(labelF)) if type(in_) is FloatType else result.append(self.jvm.emitIFICMPLT(labelF))
        elif op == "<":
            result.append(self.jvm.emitIFGE(labelF)) if type(in_) is FloatType else result.append(self.jvm.emitIFICMPGE(labelF))
        elif op == "<=":
            result.append(self.jvm.emitIFGT(labelF)) if type(in_) is FloatType else result.append(self.jvm.emitIFICMPGT(labelF))
        elif op == "!=":
            result.append(self.jvm.emitIFEQ(labelF)) if type(in_) is FloatType else result.append(self.jvm.emitIFICMPEQ(labelF))
        elif op == "==":
            result.append(self.jvm.emitIFNE(labelF)) if type(in_) is FloatType else result.append(self.jvm.emitIFICMPNE(labelF))
    ...
    *   @param in the code to be printed out
    ...
    def emitINITNEWARRAY(self, name, frame, varType, varSize=None, varKind="global"):
        result = []
        ...
        if varKind == "local":
            ...
            result.append(self.jvm.emitPUTSTATIC(name, self.getJVMType(cgen.ArrayPointerType(varType))))
        return ''.join(result)

    def emitNEWINIT(self, name):
        result = self.jvm.emitNEW(name)
        return ''.join(result)
